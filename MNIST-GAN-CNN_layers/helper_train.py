...
def train_gan_v1(num_epochs, model, optimizer_gen, optimizer_discr, 
    ...
    # of the generator
    fixed_noise = torch.randn(64, latent_dim, device=device) # format NCHW
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (features, _) in enumerate(train_loader):
            ...
            # real images
            real_images = features.view(-1, 784).to(device)
            real_labels = torch.ones(batch_size, device=device) # real label = 1

            # generated (fake) images
            noise = torch.randn(batch_size, latent_dim, device=device)  # format NCHW
            ...
            discr_pred_real = model.discriminator_forward((real_images).view(batch_size, 1, 28, 28)) # Nx1 -> N
